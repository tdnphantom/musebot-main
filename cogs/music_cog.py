...
class Music(commands.Cog):
    def __init__(self, bot):
        ...
        self.LINK_LIST = ('www.youtube.com', 'youtube.com', 'youtu.be', 'm.youtube.com', 'www.soundcloud.com', 'soundcloud.com','https://open.spotify.com/')
    ...
    async def play_music(self, ctx):
        if len(self.queue) > 0 or self.loop:
            ...
            if not self.loop:
                ...
                self.queue.pop(0)
            self.voice_channel.play(discord.FFmpegPCMAudio(self.current_song['source'], **self.FFMPEG_OPTIONS), after=lambda x: asyncio.run_coroutine_threadsafe(self.play_music(ctx), self.bot.loop))
        elif len(self.queue) == 0 or not self.voice_channel.is_playing():
            self.playing = False
            
    async def load_playlist(self, ctx, link):
        songs = []
        source = await self.bot.loop.run_in_executor(None, self.get_info, self.YDL_OPTIONS_PLAYLIST_LENGTH, link)
        playlist_length = source['playlist_count']
        for i in range(playlist_length):
            source = await self.bot.loop.run_in_executor(None, self.get_info, {'format': 'bestaudio', 'noplaylist': 'False', 'playliststart': i+1, 'playlistend': i+1}, link)
            results = source['entries'][0]
            song = {'source': results['url'], 'title': results['title']}
            self.queue.append(song)
            songs.append(song)
    ...
    async def pause(self, ctx):
        if await self.user_is_connected(ctx) and self.voice_channel.is_connected() and self.playing and self.voice_channel.is_playing():
            await ctx.send("```Stop right there, you criminal scum!```")
            self.voice_channel.pause()

    @commands.command(pass_context=True)
    async def resume(self, ctx):
        if await self.user_is_connected(ctx) and self.voice_channel.is_connected() and self.playing and self.voice_channel.is_paused():
